...
Процедура СписокНоменклатурыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ДобавитьУникальнуюПозицию(ДанныеСтроки.Номенклатура, ДанныеСтроки.Цена);
...
Процедура ЗаполнитьИзбранныеТоварыКлиента()
	...
	СчетчикДинамическихЭлементов = 0;
	
	Пока Выборка.Следующий() Цикл
		
		ТекущаяСтрока = ДинамическиеЭлементы[СчетчикДинамическихЭлементов];
		ТекущаяСтрока.Активность = Истина;
		ТекущаяСтрока.Номенклатура = Выборка.Номенклатура;
		
		СчетчикДинамическихЭлементов = СчетчикДинамическихЭлементов + 1;
...
Функция ЗаписатьПродажуНаСервере()
	...
	ДокОбъект.ПозицияПродажи.Загрузить(ПозицииПродажи.Выгрузить());
