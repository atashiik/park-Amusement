...
Процедура Билет(ТабДок, Ссылка) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ПараметрыОбластей.Вставить("Дата", Формат(Выборка.Дата, "ДЛФ=D;"));
		ПараметрыОбластей.Вставить("Номер", УдалитьЛидирующиеНули(Выборка.Номер));
		
		ОбластьЗаголовок.Параметры.Заполнить(ПараметрыОбластей);
...
Процедура ПеренестиНоменклатуруВТабличнуюЧасть() Экспорт
 	...
 	Запрос.Текст = 
 		...
 		|ИЗ
 		|	Документ.ПродажаБилета КАК ПродажаБилета
 		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПродажаБилета.ПозицияПродажи КАК ПродажаБилетаПозицияПродажи
 		|		ПО ПродажаБилетаПозицияПродажи.Ссылка = ПродажаБилета.Ссылка
 		|		И ПродажаБилетаПозицияПродажи.НомерСтроки = 1
 		|ГДЕ
 		|	ПродажаБилета.УдалитьНоменклатура <> ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
 		|	И ПродажаБилетаПозицияПродажи.Ссылка ЕСТЬ NULL";
 	...
 	Пока Выборка.Следующий() Цикл
 		...
 		Строка = ДокОбъект.ПозицияПродажи.Добавить();
